...
def calc_asset_maturity(type_of_article: str) -> str:
    if type_of_article is None:
        return None

    type_of_article = type_of_article.lower()
...
if __name__ == '__main__':
    ...
    for row in infile:
        a = Article(
            ...
            parse_category(row['categories']) if row.get(
    ...
    for i in reversed(range(len(records))):
        # make the target oobject
        n = copy.deepcopy(c19nav)

        # delete things we deemed unworthy
        ...
        for cat in records[i].category:
            print(type(n['taxonomy']))
            if cat2tax.get(cat.title, None) is not None:
                n['taxonomy'].add(cat2tax[cat.title])
                tmp_mature = calc_asset_maturity(cat2tax[cat.title])

                if tmp_mature is not None:
                    if int(re.findall('[0-9]+', tmp_mature)[0]) > int(re.findall('[0-9]+', n['asset_maturity'])[0]):
                        n['asset_maturity'] = tmp_mature

            tmp_mature = calc_asset_maturity(records[i].info.type_of_article)
            if records[i].info is not None and tmp_mature is not None:
                if n['asset_maturity'] is None:
                    n['asset_maturity'] = tmp_mature
                print(n['asset_maturity'])
                if int(re.findall('[0-9]+', tmp_mature)[0]) > int(re.findall('[0-9]+', n['asset_maturity'])[0]):
                    n['asset_maturity'] = tmp_mature

            # short things are classified wrong all the time, setting limit to reduce error rate
            if len(records[i].abstract) < 100:
                n['language'] = 'en'
            else:
                n['language'] = langid.classify(records[i].abstract)[0]

            # add the stuff that is 1:1
            n['title'] = records[i].title
            n['author'] = records[i].authors
            n['year'] = records[i].year
            n['description'] = records[i].abstract
